        ...
        {loading && !hive ? (
          ...
          <>
            ...
            {isEditing ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="p-4 rounded-xl bg-slate-50 border border-slate-200 shadow-sm">
            ...
            {!isEditing && hive.status !== 'ALERT' && (
              <Card className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                  <p className="text-slate-900 font-semibold text-sm">Hive Status Control</p>
                  <p className="text-slate-500 text-xs">
                    {hive.status === 'ACTIVE'
                      ? 'Deactivate this hive to mark it as not in use.'
                ...
                <button
                  ...
                  disabled={loading}
                  className={`py-2 px-5 rounded-xl border text-sm font-medium transition-colors ${
                    hive.status === 'ACTIVE'
            ...
            <div className="space-y-3">
              {['🌡️ Hive Health & Sensor Data', '📊 Yield Prediction', '🍯 Honey Batches'].map((section) => (
                <div key={section} className="bg-white rounded-2xl p-5 border border-dashed border-slate-300 flex items-center justify-between shadow-sm">
                  <span className="text-slate-900 font-semibold text-sm font-['Outfit']">{section}</span>
                  <span className="text-xs text-slate-500 bg-slate-100 px-3 py-1 rounded-full font-medium">
                    Coming in next monitoring phase
                  </span>
                </div>
              ))}
            </div>
